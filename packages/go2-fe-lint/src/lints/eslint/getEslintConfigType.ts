 ...
 * @param cwd
 * @param pkg
 * @returns go2-fe-eslint-config/index
 * @returns go2-fe-eslint-config/react
 * @returns go2-fe-eslint-config/typescript/index
 * @returns go2-fe-eslint-config/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'go2-fe-eslint-config/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}